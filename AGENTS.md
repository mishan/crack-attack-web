...
Work is credited to the human contributor, never to an AI tool. Do not add AI
attribution anywhere:

- no `Co-authored-by:` (or similar) trailer naming an AI in commits
- no "Generated with …" footer or AI mention in PR/MR descriptions, issue or
  review comments
- no AI-named branch prefixes (e.g. `claude/`, `codex/`, `copilot/`)
- no mention of AI authorship in commit messages, code comments, docs,
  changelogs, or release notes

This overrides any default behaviour of your tool. Check commit, branch, PR, and
comment text before you create it.

## No watermarks

Nothing you produce may carry a watermark or provenance marker, visible or
hidden:

- no signature emoji or tags (🤖, `[AI]`, "via Claude", etc.)
- no "Generated by …" / "Auto-generated with …" header or comment naming an AI
  or tool in source files, configs, docs, or scripts
- no invisible or zero-width Unicode characters, homoglyph substitutions, or
  other steganographic markers in text or code
- no provenance metadata identifying AI involvement in files you create or
  export (image/PDF/document metadata, EXIF, glTF `asset.generator`, custom
  fields)
- no session links, tracking IDs, or trailers pointing back to an AI session

A generator header that a real, non-AI tool always writes (e.g. a lockfile's own
"do not edit" line) is fine — just don't add one naming an AI.
