...
describe('GarbageGenerator solo dealing + drop', () => {
  ...
  // Deal a 1x3 normal garbage locally, then step until it drops; return the tick.
  const dropTick = (seed: number): number => {
    const { clock, gg, gm } = newRig(seed);
    gg.comboElimination(comboWith({ magnitude: 4 }));
    for (let t = 1; t <= 1000; t++) {
      clock.time_step = t;
      gg.timeStep();
      if (gm.garbage_count === 1) return t;
    }
    throw new Error('garbage never dropped');
  };

  it('drops on the same tick for the same seed (deterministic jitter)', () => {
    expect(dropTick(777)).toBe(dropTick(777));
  });

  it('drop tick falls within the jittered delay window', () => {
    // alarm = 0 + (AVG - SPREAD/2) + number(SPREAD) in [280, 319]; it drops on
    // the first tick strictly greater than the alarm, i.e. [281, 320].
    const t = dropTick(777);
    expect(t).toBeGreaterThan(GC_AVERAGE_GARBAGE_DROP_DELAY - GC_SPREAD_GARBAGE_DROP_DELAY / 2);
    expect(t).toBeLessThanOrEqual(GC_AVERAGE_GARBAGE_DROP_DELAY + GC_SPREAD_GARBAGE_DROP_DELAY / 2);
  });

  it('timeStep throws instead of reading past the queue when waiting_count is out of sync', () => {
    const { gg } = newRig();
    (gg as unknown as { waiting_count: number }).waiting_count = 3;
    expect(() => gg.timeStep()).toThrow(/out of sync/);
  });

  it('addToQueue rejects non-positive normal dimensions at ingress', () => {
    const { gg } = newRig();
    expect(() => gg.addToQueue(0, 3, GF_NORMAL, 0)).toThrow(RangeError);
    expect(() => gg.addToQueue(1, -1, GF_NORMAL, 0)).toThrow(RangeError);
    expect(gg.waitingCount).toBe(0);
  });
...
describe('ComboManager', () => {
  ...
  it('throws on a double-free of a combo', () => {
    const { cm } = newRig();
    const combo = cm.newComboTabulator();
    cm.deleteComboTabulator(combo);
    expect(() => cm.deleteComboTabulator(combo)).toThrow(/double-free/);
  });
  ...
  it('fails fast when the combo store is exhausted', () => {
    const { cm } = newRig();
    for (let i = 0; i < GC_COMBO_TABULATOR_STORE_SIZE; i++) cm.newComboTabulator();
    expect(() => cm.newComboTabulator()).toThrow(/exhausted/);
  });
