...
export class GarbageGenerator {
  ...
  addToQueue(height: number, width: number, flavor: number, stamp: number): void {
    if (!garbageIsSpecialFlavor(flavor)) {
      // Validate at ingress so a bad inbound (netcode/AI) height/width fails
      // here rather than later inside timeStep, where it would desync the queue.
      if (height <= 0 || width <= 0) {
        throw new RangeError(
          `addToQueue: garbage dimensions must be positive (height ${height}, width ${width})`,
        );
      }
      this.dealLocalGarbage(height, width, flavor, stamp);
    } else {
      this.dealSpecialLocalGarbage(flavor, stamp);
    }
  }
  ...
  timeStep(): void {
    ...
    // Scan the whole fixed queue (not just until c hits 0): this processes every
    // active entry even if waiting_count is undercounted, and lets c go negative
    // so the final check catches both under- and over-count corruption.
    for (let n = 0; n < GC_GARBAGE_QUEUE_SIZE; n++) {
    ...
    if (c !== 0) {
      throw new Error(
        `GarbageGenerator.timeStep: waiting_count (${this.waiting_count}) out of sync with the queue`,
      );
    }
  }
  ...
  private dealLocalGarbage(height: number, width: number, flavor: number, timeStamp: number): void {
    ...
    // Bounded free-slot search: with waiting_count < size a free slot must
    // exist, but bound and fail fast if the count is ever out of sync.
    let i = 0;
    while (i < GC_GARBAGE_QUEUE_SIZE && this.garbage_queue[i]!.active) i++;
    if (i === GC_GARBAGE_QUEUE_SIZE) {
      throw new Error('GarbageGenerator: no free queue slot despite waiting_count < size');
    }
  ...
  private dealSpecialLocalGarbage(flavor: number, timeStamp: number): void {
    switch (flavor) {
      ...
      default:
        // garbageIsSpecialFlavor accepts any non-normal flavor, so an invalid
        // inbound value would otherwise be silently dropped — fail fast instead.
        throw new RangeError(`dealSpecialLocalGarbage: unknown special flavor ${flavor}`);
    }
  }
}
